---
description: >-
  睡眠服务是一项很重要的服务，也是一个非常复杂的功能，睡眠是在服务器计算，根据大数据来分析，会持续更新。部分戒指支持gomore睡眠算法，这个可以在戒指端计算，根据复合指令的返回字段gomoreSleep来判断是否支持。
icon: face-sleeping
---
...
Gomore睡眠是集成在戒指里，可以通过复合指令的返回字段（gomoreSleep）来判断戒指是否支持。可以直接通过指令，获取用户睡眠信息，用户可以保存睡眠信息到自己服务器，也可以调用我们的接口，保存到我们云端。如果要保证GoMore算法可以正常使用，需要进行授权。
...
建议流程是，在获取了历史记录以后，获取gomore睡眠，设置用户信息，因为设置了用户信息，也只会在下一次睡眠产生的时候生效，部分戒指先设置 用户信息，再获取睡眠，会出异常
 ...
 * gomore多段睡眠合并逻辑
 *
 * <p>合并范围：resType=0 且 type=1 的睡眠总览
 * <p>不处理：resType=2 完全不动、不参与、不合并
 * <p>夜间定义：20:00 ~ 次日 10:00
 * <p>一段睡眠：不合并
 * <p>多天睡眠：按天独立分组，互不干扰
 * <p>分期点：60 秒 = 1 个点
 * <p>间隔空白：自动补 0（清醒）
 * <p>合并输出：新增 1 条总览 + 1 条分期
 * <p>结构：分期通过 sleepOverviewId 关联总览
 * <p>原始数据：完全保留、不修改、不删除
 * <p>夜间长睡合并工具（只合并 type=1 长睡，小睡 type=2 不参与任何合并/计算）
 * <p>合并后会生成 1 条总览 + 1 条完整睡眠分期，标记 sleepOverviewId=-1
 * <p>去掉间隔判断：同天夜间（20:00~次日10:00）所有睡眠，不管间隔多久，全部合并为一条
 *
 * <p><b>合并规则说明：</b>
 * <ul>
 *   <li><b>去重规则：</b>同一夜间内，相同开始时间（startTs）的多条睡眠，只保留结束时间（endTs）最晚的那一条，
 *       其余丢弃不参与合并</li>
 *   <li><b>排序规则：</b>去重后的睡眠段按开始时间（startTs）升序排列，依次处理</li>
 *   <li><b>重叠处理规则：</b>多条睡眠在时间轴上发生重叠时，<b>不进行数值融合（如取平均、取最优等）</b>，
 *       采用 <b>后来覆盖先前（后写覆盖）</b> 的策略，即后处理的睡眠分期数据直接覆盖先处理的睡眠在重叠区间的分期数据。
 *       因此合并结果依赖于 group 列表的处理顺序（已按 startTs 升序排列）</li>
 *   <li><b>空白处理规则：</b>没有任何睡眠覆盖的时间区间，自动填充 0（代表清醒状态）</li>
 *   <li><b>时间轴跨度：</b>从最早睡眠的 startTs 到最晚睡眠的 endTs，覆盖整个夜间区间</li>
 * </ul>
 *
 * <p><b>合并示例：</b>
 * <pre>
 * 睡眠A：22:00~23:00，分期数据为 [A0, A1, A2, ..., A59]
 * 睡眠B：22:30~23:30，分期数据为 [B0, B1, B2, ..., B59]
 *
 * 合并结果时间轴：22:00~23:30（90分钟）
 * 填充过程：
 *   - 先处理睡眠A：填充位置 0~59
 *   - 再处理睡眠B：从位置 30 开始填充，覆盖位置 30~59
 * 最终结果：[A0..A29][B0..B59]
 *            ↑ 22:00~22:30 保留A的数据
 *               ↑ 22:30~23:30 使用B的数据（重叠部分被B覆盖）
 * </pre>
...
4.睡眠分期是60s产生一条，如果要绘图，可以开始时间加60s算结束时间

### 服务器相关接口

看公司需求，如果需要保存到自己数据库，这一步可以省略，Gomore算法返回的数据会有多条，前几条是分期，后边一条是汇总，所以如果需要正常显示睡眠，需要取睡眠开始时间相同，结束时间最迟的那一条
