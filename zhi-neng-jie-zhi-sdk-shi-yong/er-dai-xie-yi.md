...
```java
    LmAPILite.APP_CONNECT(long timeMillis);//秒级时间戳
```
...
<mark style="color:$danger;">时间戳的作用是保证服务器端数据完整性。有以下情况，会导致数据不完整，比如获取到戒指的数据A，上传到服务端的时候，网络错误，没上传上去，或者在其他设备上同步过历史数据A，下次同步的时候，戒指默认A已经同步过了，就只会上传A之后的数据，导致A丢失。如果设置了A之前数据的时间戳，就会重新把A拉取出来，如果数据是传递到服务端，可以将最后一条记录的时间戳传进入，拉取之后的数据上传，这样服务端就是完整数据</mark>
...
```java
    LmAPILite.APP_REFRESH(long timeMillis);//秒级时间戳
```
